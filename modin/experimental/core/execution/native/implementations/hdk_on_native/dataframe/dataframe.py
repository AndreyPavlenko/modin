...
class HdkOnNativeDataframe(PandasDataframe):
    ...
    def _union_all(
        ...
        HdkOnNativeDataframe
            The new frame.
        """
        # In case of different columns, HDK performs slowly and
        # supports only numeric column types.
        # See https://github.com/intel-ai/hdk/issues/182
        if len(other_modin_frames) == 0 or any(
            (len(f.columns) != len(self.columns) for f in other_modin_frames)
            or any(f.columns != self.columns)
            for f in other_modin_frames
        ):
        ...
        for frame in [self] + other_modin_frames:
            ...
            if not ignore_index:
                if frame._index_cols:
                    ...
                    aligned_index = [IDX_COL_NAME]
                    exprs[IDX_COL_NAME] = frame.ref(ROWID_COL_NAME)
    ...
    def _union_all_arrow(self, other_modin_frames, join, sort, ignore_index):
        ...
        if len(col_fields) == 0:
            ...
            if not ignore_index:
                if all(f.index_cols is None for f in frames):
                    # All frames have a trivial index
                    idx_cols = self._mangle_index_names(self.index.names)
                    idx_ranges = list(range(len(frames[0].table)))
                    for frame in frames[1:]:
                        idx_ranges.extend(range(len(frame.table)))
                else:
            ...
            if idx_ranges is not None:
                union = union.add_column(0, idx_cols[0], pyarrow.array(idx_ranges))
    ...
    def to_pandas(self):
        ...
        if len(df.columns) != len(self.columns):
            ...
            if (self._index_cache is not None) and (
                (self._index_cache.name is not None) or (len(self.columns) == 0)
            ):
                # Preserving non-materialized index if either it has a name or the frame
                # doesn't have columns and, thus, the index length could not be calculated.
                df.index = self._index_cache.copy()
